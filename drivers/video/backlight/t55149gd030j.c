...
#include "/home/ebosch/Kernels_IMASD/linux-odroid-public/arch/arm/plat-samsung/include/plat/gpio-cfg.h"
...
#else
int t55149gd030j_reset(void)
{
	int err = 0;
	
	
	err = gpio_request_one(28, GPIOF_OUT_INIT_HIGH, "GPC1");
	if (err) {
...
static void t55149gd030j_set_ID(int id_val)
{
	...
	gpio_set_value(228, id_val & 0x1); //SPI ID
}

void t55149gd030j_set_rgbmode(void)
{
	//LCD IM1 signal assigned to GPX1[7]
	gpio_set_value(227, 1); //RGB mode
	t55149gd030j_set_ID(1);
}

void t55149gd030j_set_spimode(void)
{
	//LCD IM1 signal assigned to GPX1[7]
	gpio_set_value(227, 0); //SPI mode
...
{
	s3c_gpio_cfgall_range(40, 8, S3C_GPIO_SFN(0), S3C_GPIO_PULL_DOWN);
	s3c_gpio_cfgall_range(48, 8, S3C_GPIO_SFN(0), S3C_GPIO_PULL_DOWN);
	s3c_gpio_cfgall_range(56, 5, S3C_GPIO_SFN(0), S3C_GPIO_PULL_DOWN);
	s3c_gpio_cfgall_range(63, 2, S3C_GPIO_SFN(0), S3C_GPIO_PULL_DOWN);
	s3c_gpio_cfgall_range(64, 4, S3C_GPIO_SFN(0), S3C_GPIO_PULL_DOWN);
}
